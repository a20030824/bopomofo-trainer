...
export function serializeCellComparisonsCsv(
  ...
  const rows: string[] = [[
    ...
    "executable_rounds", "fallback_rate", "failure_rate", "metric", "direction",
    "role", "status", "cell_mean", "baseline_mean", "absolute_delta",
    "relative_delta", "improvement", "relative_improvement", "material_improvement",
    "material_regression", "cell_samples", "cell_missing", "baseline_samples",
    "baseline_missing",
  ].join(",")];
  for (const comparison of analysis.comparisons) {
    for (const metric of comparison.metrics) {
      rows.push([
        ...
        comparison.fallbackRate, comparison.failureRate,
...
export function serializeAxisSummariesCsv(
  ...
  const rows: string[] = [[
    ...
    "fallback_rate", "failure_rate", "metric", "samples", "missing", "mean",
    "standard_deviation", "confidence_95_low", "confidence_95_high", "reason",
  ].join(",")];
  for (const summary of analysis.axisSummaries) {
    for (const [metric, stats] of Object.entries(summary.metrics)) {
      rows.push([
        ...
        summary.cellCount, summary.runCount, summary.fallbackRate, summary.failureRate,
...
function axisOverview(analysis: RelationalExperimentAnalysis): string[] {
  ...
  return summaries.map((item) =>
    ...
    + ` | ${number(item.fallbackRate)} | ${number(item.failureRate)}`
...
export function serializeRelationalAnalysisMarkdown(
  ...
  const lines = [
    ...
    "",
    "| Axis | Level | Balanced | Fallback | Failure | Weakness delay | Coverage | Cost/improvement |",
    "|---|---|---|---:|---:|---:|---:|---:|",
