...
describe("thin product real catalog integration", () => {
  it("reserves five held-out entries while preserving practice support", async () => {
    ...
    const environment = createProductEnvironment(partition);
    ...
    expect(state.round.exercise.entries).toHaveLength(6);
  });
});
